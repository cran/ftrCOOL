...
#' or indices in the DI_DNA file. This parameter could be a vector or a list of dinucleotide indices.
...
AutoCorDiNUC_DNA<-function(seqs,selectedIdx=c("Rise", "Roll", "Shift", "Slide", "Tilt", "Twist"),maxlag=3,threshold=1,type=c("Moran","Geary","NormalizeMBorto","AC","CC","ACC"),label=c()){
  ...
  if(is.list(selectedIdx))
  {
    for(i in 1:length(selectedIdx)){
      if(is.numeric(selectedIdx[[i]])){
        ...
        stop('ERROR: selectedIdx should be Ids or indices of the selected properties in the di-nucleotide index file')
      }
    }
  ...
  if("CC" %in% type){
    featureMatrix<-CC_CorDi(selectedProp = selectedIdxNames ,maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("CC",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("ACC" %in% type){
    featureMatrix<-ACC_CorDi(selectedProp = selectedIdxNames ,maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
