...
#'
#' @references Wei,L., Su,R., Luan,S., Liao,Z., Manavalan,B., Zou,Q. and Shi,X. Iterative feature representations improve N4-methylcytosine site prediction. Bioinformatics,  (2019).
#'
...
#'
#' @param trainSeq is a fasta file with ribonucleotide sequences. Each sequence starts
#' with a '>' character. Also it could be a string vector such that each element is a ribonucleotide sequence. Eaxh sequence in the training set
...
KNN_RNA <- function(seqs,trainSeq,percent=30,labeltr=c(),label=c())
{
  ...
  if(length(trainSeq)==1&&file.exists(trainSeq)){
    trainSeq<-fa.read(trainSeq,alphabet="rna")
    trseqs_Lab<-alphabetCheck(trainSeq,alphabet = "rna",label=labeltr)
