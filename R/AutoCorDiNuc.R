#' Di Nucleotide Autocorrelation-Autocovariance
#'
#'
...
#' or indices in the nuIdx2 file. This parameter could be a vector or a list of dinucleotide indices.
#'
#'
...
AutoCorDiNuc<-function(seqs,selectedNucIdx,maxlag=3,threshold=1,type=c("Moran","Geary","NormalizeMBorto","AC","CC","ACC"),label=c()){
  ...
  if(is.list(selectedNucIdx))
  {
    for(i in 1:length(selectedNucIdx)){
      if(is.numeric(selectedNucIdx[[i]])){
        ...
        stop('ERROR: selecteNucIdx should be Ids or indices of the selected properties in the di-nucleotide index file')
      }
    }
  ...
  if("Moran" %in% type){
    featureMatrix<-Moran_corDi(maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("Mo",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("Geary" %in% type){
    featureMatrix<-Geary_corDi(maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("Ge",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("NormalizeMBorto" %in% type){
    featureMatrix<-NormalizeMBorto_corDi(maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("No",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("AC" %in% type){
    featureMatrix<-AC_CorDi(maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("AC",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("CC" %in% type){
    featureMatrix<-CC_CorDi(selectedProp = selectedNucIdxNames ,maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
    colnames(featureMatrix)<-paste("CC",colnames(featureMatrix))
    totalfeatureMatrix<-cbind(totalfeatureMatrix,featureMatrix)
  }
  if("ACC" %in% type){
    featureMatrix<-ACC_CorDi(selectedProp = selectedNucIdxNames ,maxLag = maxlag,aaIdx = aaIdx,pPrim = Pprim,numSeqs=numSeqs,lenSeqs=lenSeqs,Dimer = Di)
...
Moran_corDi<-function(maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
...
Geary_corDi<-function(maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
  totalMatrix<-matrix(0,nrow = numSeqs,ncol = (nrow(aaIdx)*maxLag))
  tempName<-rep(1:maxLag,nrow(aaIdx))
  tempName<-paste0("d=",tempName)
  temp2Name<-rep(row.names(aaIdx),each=maxLag)
...
NormalizeMBorto_corDi<-function(maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
  totalMatrix<-matrix(0,nrow = numSeqs,ncol = (nrow(aaIdx)*maxLag))
  tempName<-rep(1:maxLag,nrow(aaIdx))
  tempName<-paste0("d=",tempName)
  temp2Name<-rep(row.names(aaIdx),each=maxLag)
...
AC_CorDi<- function(maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
...
CC_CorDi<- function(selectedProp, maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
  ...
  for (i in 1:length(selectedProp)){
    if (length(selectedProp[[i]])>2){
      selectedProp[[i]]<-unique(selectedProp[[i]])
      comb<-combn(selectedProp[[i]],2)
...
ACC_CorDi<- function(selectedProp, maxLag=30,aaIdx,pPrim,numSeqs,lenSeqs,Dimer){
  ...
  for (i in 1:length(selectedProp)){
    if (length(selectedProp[[i]])>2){
      vect<-selectedProp[[i]]
      names(vect)<-names(selectedProp[[i]])
      vect<-unique(vect)
      comb<-combn(vect,2)
      newProp<-split(comb, rep(1:ncol(comb), each = nrow(comb)))
      selectedProp[i]<-NULL
