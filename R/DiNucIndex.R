...
#'
#'
#' @details There are 125 physicochemical indexes in the dinucleotide database.
#'
...
#'
#' @param selectedNucIdx DiNucIndex function works based on physicochemical properties. Users, select the properties by their ids
#' or indexes in dinucleotide file.
#'
#' @param standardized is a logical parameter. If it is set to TRUE, dinucleotide indices will be in the standard format.
...
#' matrix for sequences with the same length such that the number of columns is (sequence length)*(number of selected di-nucleotide indexes)
...
DiNucIndex <- function(seqs,selectedNucIdx,standardized=TRUE,threshold=1,label=c(),outFormat="mat",outputFileDist="")
{
